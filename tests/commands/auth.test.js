...
describe('Auth command - unit', () => {
  ...
  test('should start a authentication flow and print the oauth url when a clientSecret is provided', async () => {
    ...
    const obtainedCredentials = JSON.parse(
      fs.readFileSync(defaults.STORAGE_FILE)
    );
    expect(obtainedCredentials.refresh_token).toEqual(
      'woot_successfully_obtained_refresh_token'
    );
  });
