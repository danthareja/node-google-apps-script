...
module.exports = function(path) {
  return getCredentialsFromFile(path)
...
function getCredentialsFromFile(path) {
  ...
  fs.readFile(path, function(err, res) {
    if (err) return deferred.reject(errors.wrongCredentialPath);
    if (!res) return deferred.reject();

    var credentials = JSON.parse(res);
    if (!credentials.web) return deferred.reject(errors.wrongCredentialsFile);
