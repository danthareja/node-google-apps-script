...
module.exports = function auth(clientSecretPath, withWebserver) {
  ...
  return fs.lstatAsync(config.STORAGE_FILE)
    .then(function() {
      console.log('~/.gaps already exists. Remove it to re-authenticate node-google-apps-script'.green);
      return Promise.resolve();
    })
    .catch(function(err) {
      if (!clientSecretPath) {
        throw gaps.config.STORAGE_FILE + ' does not exist yet. Specify a credential file with `--auth ~/Downloads/client_secret_abcd.json`';
      }
  ...
  return fs.readFileAsync(clientSecretPath)
    ...
    .then(function(credentials) {
      if (!credentials.installed) {
        throw errors.wrongCredentialsFile;
      }
...
function authenticateWithGoogle(credentials, withWebserver) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (withWebserver) {
      redirect_uri = 'http://localhost:' + config.WEBSERVER_PORT;
    }
    ...
    if (withWebserver) {
      ...
      require('../utils/miniOauthServer')(config.WEBSERVER_PORT)
        .then(getToken)
        .catch(function(err) {
...
function saveAuthenticationConfig(credentials) {
  return fs.writeFileAsync(config.STORAGE_FILE, JSON.stringify(credentials, "", 2));
}
