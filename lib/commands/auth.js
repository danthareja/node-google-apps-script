...
module.exports = function auth(clientSecretPath, withWebserver) {
  ...
  return fs.lstatAsync(defaults.STORAGE_FILE)
    ...
    .catch(function(err) {
      if (!clientSecretPath) {
        throw gaps.defaults.STORAGE_FILE + ' does not exist yet. Specify a credential file with `--auth ~/Downloads/client_secret_abcd.json`';
      }
...
function authenticateWithGoogle(credentials, withWebserver) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (withWebserver) {
      ...
      require('../utils/miniOauthServer')(defaults.WEBSERVER_PORT)
        .then(getToken)
        .catch(function(err) {
