...
2. Run `gapps init <projectId> [subdir]` (see Usage below for details)
...
3. Awesome! Now, to upload changes, run `gapps upload`. You should then be able
...
1. `gapps upload [subdir]` will upload files to Google Drive from an (optional)
subdirectory.
...
- Add `.manifest.json` to `.gitignore`
  - There is no need to push it to GitHub as the manifest is only needed locally
    for gapps to work correctly.
- Don't use `gapps download`
...
Run `gapps init <projectId> [subdir]` to link your local files to that
...
local files. `gapps init <projectId> src` for example.

### Upload a project
Run `gapps upload [subdir]` (alias: push) from the root of your project's
...
instead of the root directory. `gapps upload src`, for example.
...
server's by calling `gapps sync`. Then you will be able to upload
changes, overwriting the files on the server.

### Download a project
**It is not recommended to use `gapps download`; see `Development Workflow and Usage` for best practices.**

Run `gapps download <projectId> [subdir]` to download an existing Google Apps Script
project.
